...
- [x] 未定義接頭辞は推測せず、接頭辞ごとにユーザーへ個別確認する（途中からユーザーの手入力優先へ切替）
- [x] 更新結果を検証して報告する（全8,204行中E列空欄89件、既存値を保護、回帰テスト30件成功）
...
- [x] 接頭辞nkを二光社としてベンダー対応表・全商品リストE列へ反映する（旧中川政七を114行訂正、検証失敗0件）
- [x] 接頭辞wcを若兆としてベンダー対応表・全商品リストE列へ反映する（25行、検証失敗0件）
- [x] 接頭辞ieを家田紙工としてベンダー対応表・全商品リストE列へ反映する（24行、検証失敗0件）
- [x] 接頭辞mkを前謙としてベンダー対応表・全商品リストE列へ反映する（9行、検証失敗0件）
- [x] 接頭辞yyをワイヨットとしてベンダー対応表・既存E列の誤った仕入れ元を訂正する（旧山善を268行訂正、検証失敗0件）
- [x] 未確認の接頭辞sgに誤入力した「ソニック」11行を空欄へ戻し、ベンダー対応表から除外する（検証失敗0件）
- [x] 手入力済みの接頭辞zcは自動更新・ベンダー対応表への追加を行わない
- [x] 手入力後の全商品リストE列を再読込みし、既存値を変更せず最新状態を確認する（空欄172件、既存値を変更しないことを確認）
- [x] 接頭辞msをミランダスタイルとしてベンダー対応表・全商品リストE列へ反映する（17行、検証失敗0件）
- [x] 接頭辞iqを一久としてベンダー対応表・全商品リストE列へ反映する（21行、検証失敗0件）
- [x] 接頭辞scを瀬戸刃物としてベンダー対応表・全商品リストE列へ反映する（4行、検証失敗0件）
- [x] 接頭辞yiをユミトルインポートとしてベンダー対応表・既存E列の誤った仕入れ元を訂正する（旧山一を290行訂正、検証失敗0件）
- [x] 接頭辞twを十二堂としてベンダー対応表・既存E列の誤った仕入れ元を訂正する（旧TEN-TWOを47行訂正、検証失敗0件）

## アプリ・Gmail連携の停止
- [x] 稼働中のGmail定期処理と認可設定を確認する（Heartbeat 1件、Gmailコネクタは無効）
- [x] Gmail自動取り込みを停止する（Heartbeat `gmail-fetch` を停止）
- [x] Gmail OAuth認可情報をアプリ環境から無効化する（既存の値は使わず、実行時無効化スイッチで遮断する）
- [x] Gmail連携無効化スイッチを導入し、設定済み認可情報が残っていてもGmail APIを呼び出せないことをテストする
- [x] アプリの公開稼働を停止する（APP_SUSPENDED=trueで全HTTPアクセスを503へ切替）
- [ ] 本番公開後の停止状態を検証して報告する
