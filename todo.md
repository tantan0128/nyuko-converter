...
## UI改善3点（2026-07-23）
- [x] GmailJobs.tsxの件数バッジ（変換成功件数）を大きく目立たせる
- [x] GmailJobs.tsxの未登録詳細モーダルにその場登録ボタンを追加
- [x] Main.tsxの処理モード説明文を日本語に統一（英語UPPERCASE除去）
- [x] Main.tsxの処理結果欄を結果なし時はコンパクト・結果あり時は大きく表示

## GmailジョブCSVに仕入れ元名を追加（2026-07-23）
- [x] GmailJobs.tsxのCSVダウンロード時に備考列へ仕入れ元名を追加

## 十二堂CSVモード追加（2026-07-23）
- [x] バックエンドに十二堂CSV処理エンドポイントを追加（Shift-JIS対応、十二堂コード→助ネココード変換）
- [x] Main.tsxに十二堂CSVモードUIを追加（CSVファイルアップロード→変換→ダウンロード）
...
## 納品書DXの納品書コード取り込み
- [x] 納品書DX1.0.xlsxのカスタム商品コードを全商品リストの自社商品コードへ対応付ける
- [x] 納品書DXの納品書コードだけを全商品リストD列へ重複なく追記する
- [x] 更新件数・未一致件数・重複除外件数を検証して報告する
- [x] 納品書DX反映結果をユーザーへ報告する（更新45行、既存3,055件、未一致1,235件、検証失敗0件）

## 全商品リストの仕入れ元入力
- [x] ベンダー対応表と自社商品コード接頭辞を照合する
- [x] 定義済み接頭辞の仕入れ元を全商品リストE列へ入力する（6,128行を更新、既存5行は保持）
- [ ] 未定義接頭辞は推測せず、接頭辞ごとにユーザーへ個別確認する
- [ ] 更新結果を検証して報告する
- [x] 接頭辞crをコレドとしてベンダー対応表・全商品リストE列へ反映する（53行、検証失敗0件）
- [x] 接頭辞brをブランシュアソシエとしてベンダー対応表・全商品リストE列へ反映する（51行、検証失敗0件）
- [x] 接頭辞oiを片力商事としてベンダー対応表・全商品リストE列へ反映する（既存の大泉物産を81行訂正、検証失敗0件）
- [x] 接頭辞knをカク仲としてベンダー対応表・全商品リストE列へ反映する（37行、検証失敗0件）
- [x] 接頭辞nfをネ・ルフレとしてベンダー対応表・全商品リストE列へ反映する（31行、検証失敗0件）
- [x] 接頭辞ktを公長斎小菅としてベンダー対応表・全商品リストE列へ反映する（30行、検証失敗0件）
- [x] 接頭辞kwをカワイとしてベンダー対応表・全商品リストE列へ反映する（30行、検証失敗0件）
- [x] 接頭辞slをサンライフとしてベンダー対応表・全商品リストE列へ反映する（29行、検証失敗0件）
- [x] 接頭辞ksを京千としてベンダー対応表・全商品リストE列へ反映する（29行、検証失敗0件）
- [x] 接頭辞mxをメルクロスとしてベンダー対応表・既存E列の誤った仕入れ元を訂正する（105行、検証失敗0件）
- [x] 接頭辞ksのE列反映結果を監査ログで再確認する（29行一致、検証失敗0件）
